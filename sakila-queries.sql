...
SELECT customer_id, first_name, last_name, email FROM customer WHERE active=1 AND create_date BETWEEN '2006-02-01' AND '2006-02-28';
...
SELECT 	email, character_length(email) AS email_length from customer ORDER BY email_length DESC, email;

-- 13. Now retrieve top 100 customer records with the shortest email address.
SELECT 	email, character_length(email) AS email_length from customer ORDER BY email_length LIMIT 100;
